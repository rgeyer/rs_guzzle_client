...
class AuthenticationCommandsTest extends ClientCommandsBase {
    ...
    $cookiePlugin->onRequestSent(new Event(array('response' => $response)));

		$this->assertEquals(1, count($cookieJar->all('my.rightscale.com')));
	}
	...
	public function testCookieJarIsNotRFC2965Compliant() {
		...
		$this->assertEquals(0, count($cookieJar->all('my.rightscale.com')));
	}
